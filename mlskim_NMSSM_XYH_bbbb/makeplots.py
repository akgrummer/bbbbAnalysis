...
validationRegionSelection   = configFile.validationRegionSelection       
...
#  if not (os.path.exists(const.outputDirPrefix)): os.makedirs(path)
...
#  regionTag = "VR"
#  if (regionTag=="CR"): dataset.query(controlRegionSelection, inplace = True)
#  elif (regionTag=="VR"): dataset.query(validationRegionSelection, inplace = True)
...
#  DrawPlots(dataset.query(bTagSelection), dataset.query(antiBTagSelection), trainingVariables, outputDirectory, const.modelFileName, analysisBackgroundArgument, analysisClassifierArgument,regionTag)
