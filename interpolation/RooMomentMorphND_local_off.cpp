...
void RooMomentMorphND_local::initialize()
{
    ...
    if (nPar != nDim)
    {
        coutE(InputArguments) << "RooMomentMorphND_local::initialize(" << GetName() << ") ERROR: nPar != nDim"
                              << ": " << nPar << " !=" << nDim << endl;
        assert(0);
    }
    ...
    if (_setting == NonLinear || _setting == NonLinearPosFractions || _setting == NonLinearLinFractions)
    {
        ...
        for (int k = 0; k < nPdf; ++k)
        {
            std::cout << "iPdf = " << k << std::endl;
            int nperm = 0;
            for (int i = 0; i < nCombs; i++)
            {
                std::cout << "iCombination = " << i << std::endl;
                double tmpDm = 1.0;
                for (int ix = 0; ix < nPar; ix++)
                {
                    Double_t delta = dm[k][ix];
                    tmpDm *= TMath::Power(delta, static_cast<double>(output[i][ix]));
                    std::cout << "iParameter = " << ix;
                    std::cout << " distance = " << delta;
                    std::cout << " allCombinations[iCombination][iParameter] = " << output[i][ix];
