...
#ifndef ROOMOMENTMORPHND_local
#define ROOMOMENTMORPHND_local
...
class RooMomentMorphND_local : public RooAbsPdf
{
...
public:
    ...
    public:
        ...
        Grid(const RooAbsBinning &binning_x)
        {
            ...
            _nnuis.push_back(0);
            _filledGridPoints.emplace_back(std::vector<bool>(binning_x.numBins() + 1, false));
        };
        Grid(const RooAbsBinning &binning_x, const RooAbsBinning &binning_y)
        {
            ...
            _nnuis.push_back(0);
            _filledGridPoints.emplace_back(std::vector<bool>(binning_y.numBins() + 1, false));

            std::cout << __PRETTY_FUNCTION__ << __LINE__ << " " << _filledGridPoints.size() << std::endl;
        ...
        Grid(const RooAbsBinning &binning_x, const RooAbsBinning &binning_y, const RooAbsBinning &binning_z)
        {
            ...
            _nnuis.push_back(0);
            _filledGridPoints.emplace_back(std::vector<bool>(binning_z.numBins() + 1, false));
        };
        Grid(const vector<RooAbsBinning *> binnings)
        {
            for (unsigned int i = 0; i < binnings.size(); i++)
            {
                ...
                _nnuis.push_back(0);
                _filledGridPoints.emplace_back(std::vector<bool>(binnings[i]->numBins() + 1, false));
            }
        };
        ...
        vector<vector<int>> getAllCombinations();
        vector<vector<double>> getAllBoundaryCombinations();
        // void updateFilledBins();
        ...
        mutable vector<std::vector<bool>> _filledGridPoints;
    };
...
protected:
    ...
    public:
        ...
        void calculateFractions(const RooMomentMorphND_local &self, Bool_t verbose = kTRUE) const;
    };
...
public:
    ...
    RooMomentMorphND_local();
    RooMomentMorphND_local(const char *name, const char *title, RooAbsReal &_m, const RooArgList &varList,
                           const RooArgList &pdfList, const RooArgList &mrefList, Setting setting);
    RooMomentMorphND_local(const char *name, const char *title, const RooArgList &parList, const RooArgList &obsList,
                           const Grid &referenceGrid, const Setting &setting);
    RooMomentMorphND_local(const RooMomentMorphND_local &other, const char *name = 0);
    RooMomentMorphND_local(const char *name, const char *title, RooAbsReal &_m, const RooArgList &varList,
                           const RooArgList &pdfList, const TVectorD &mrefpoints, Setting setting);
    virtual ~RooMomentMorphND_local();
    virtual TObject *clone(const char *newname) const { return new RooMomentMorphND_local(*this, newname); }
...
protected:
    ...
    //  ClassDef(RooMomentMorphND_local, 1)
};
