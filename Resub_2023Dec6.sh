...
echo "Number of jobs: " "${#resubJobs2[@]}"
# ls "${resubJobs[1]}"
# for var in "${resubJobs[@]}" ; do
### BACKUP OLD JOBS
for var in "${resubJobs2[@]}" ; do
    printf "%s\n" "${var}"
    thedir=$(dirname "${var}")
    mkdir -p badMoreSignalJobs2/${thedir}/
    mv "${var}"_*.log badMoreSignalJobs2/${thedir}/
    mv "${var}"_*.stderr badMoreSignalJobs2/${thedir}/
    mv "${var}"_*.stdout badMoreSignalJobs2/${thedir}/
done
