...
def sigmaPlot( h1, h2, year, region, var, tag, label,  odir, hsig):
    h1 = h1.Clone("h1copy")
    h2 = h2.Clone("h2copy")
...
def rootplot_2samp_ratio( h1, h2, year, region, var, tag, label,  odir, hsig):
    #### get the histograms:
    h1 = h1.Clone("h1copy")
    h2 = h2.Clone("h2copy")
    hsig = hsig.Clone("hsigcopy")
    ...
    if "VR" in region:
        if "InVR" in tag:
            labelText = labelText + "Inner Half of Validation Region"
        elif "OutVR" in tag:
            labelText = labelText + "Outer Half of Validation Region"
        else:
            labelText = labelText + "Validation Region"
    if "CR" in region:    
        if "OutCR" in tag:
            labelText = labelText + "Outer Half of Control Region"
        elif "InCR" in tag:
            labelText = labelText + "Inner Half of Control Region"
        elif "toInQtr" in tag:
            labelText = labelText + "Inner Quarter of Control Region"
        elif "toOutQtr" in tag:
            labelText = labelText + "Outer Quarter of Control Region"
        elif "RightSide" in tag:
            labelText = labelText + "Right Band of Control Region"
        elif "LeftSide" in tag:
            labelText = labelText + "Left Band of Control Region"
        elif "ExcRight" in tag:
            labelText = labelText + "Control Region - Exc. Right Outer Half"
        elif "ExcLeft" in tag:
            labelText = labelText + "Control Region - Exc. Left Outer Half"
        else:
            labelText = labelText + "Control Region"
    ...
    plotlabels.SetTextSize(16)
    plotlabels.DrawLatexNDC(0.35, 0.78,label)
...
def compareUnrolled(dir_region, region, year, odir,tag, label):
    odir = odir +year
    ...
    if ("VR" in region): ifileName = "VarPlots/rootHists/%s/%sDataPlots_%s_VR/outPlotter_massGroup0.root"%(dataFileTagDir, year,tag)
    if("CR" in region): ifileName = "VarPlots/rootHists/%s/%sDataPlots_%s/outPlotter_massGroup0.root"%(dataFileTagDir, year,tag)
...
#get histograms
    ...
    #  rootplot_2samp_ratio( h_3b_weights, h_4b,  year, region, varname, tag, label,  odir, h_4b)
    sigmaPlot( h_3b_weights, h_4b,  year, region, varname, tag, label,  odir, h_4b)
