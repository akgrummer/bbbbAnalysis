...
void calculate_HG_BKGshape(int year, TString tag)
{
    ...
    TH2F *h1=(TH2F*)ifile->Get(dataset+"/"+regionVR+"/"+dataset+"_"+regionVR+"_"+varname);
    ...
    for(const auto& massGroup : theMassGroupList){
        ...
        TH1F* h1up_unrolled = UnrollPlot(h1up_rebinned);
        ...
        TH1F* h1down_unrolled = UnrollPlot(h1down_rebinned);
...
int main(int argc, char *argv[])
{
    ...
    // this is an existing folder that has already been converted to VR
