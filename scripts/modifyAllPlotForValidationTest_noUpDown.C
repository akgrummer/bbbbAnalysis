...
void modifyAllPlotForValidationTest_noUpDown(std::string tag = "v32_bkgValidation")
{
    ...
    for(const auto year : yearList)
    {
        for(const auto massGroup : massesGroupList)
        {
            ...
            std::string inputFileName = "VarPlots/rootHists/" + std::to_string(year) + "DataPlots_" + tag + "/outPlotter_massGroup" + std::to_string(massGroup) + ".root";
