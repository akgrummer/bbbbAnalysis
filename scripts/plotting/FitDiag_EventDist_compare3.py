...
def rootplot_1Dhist(h1, h2, year, idir, tag1, tag2):
    ...
    if(args.monitor != "mean"):
        ...
        h1.GetYaxis().SetRangeUser(0, h2.GetBinContent(h2.GetMaximumBin())*1.2 )
        h1.GetYaxis().SetTitle("Events per bin")
        h2.GetYaxis().SetRangeUser(0, h2.GetBinContent(h2.GetMaximumBin())*1.2 )
    ...
    c1.SaveAs("{0}/{1}vs{2}_{3}_{4}.pdf".format( odir, tag1, tag2, year, args.monitor))

def makePlotsPerYear(year, eventDistFile1, eventDistFile2):
  ...
  rootplot_1Dhist(h1, h2, year, idir, tag1, tag2)
...
makePlotsPerYear("{}".format(args.year), ifile, ifile2)
