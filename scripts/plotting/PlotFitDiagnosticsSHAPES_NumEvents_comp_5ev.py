...
def makePlot(labels, ibinMap1, ibinMap2, ibinMap3, inputFile1, inputFile2,  inputFile3):
  fittype=labels[0]
  tag = labels[1]
  year = labels[2]
  ...
  ifile3 = ROOT.TFile(inputFile3)
  ifile3.cd("data_BTagCSV_dataDriven_kinFit/selectionbJets_SignalRegion");
  hbkgIN3 = ifile3.Get("data_BTagCSV_dataDriven_kinFit/selectionbJets_SignalRegion/data_BTagCSV_dataDriven_kinFit_selectionbJets_SignalRegion_HH_kinFit_m_H2_m_Rebinned_Unrolled");
  n_EventsDist3 = hbkgIN3.GetBinContent(hbkgIN3.GetMaximumBin())
  hBkgShape_EventsDist3 = ROOT.TH1D( "hBkgShape_EventsDist", "hBkgShape_EventsDist", int(n_EventsDist3), 0, n_EventsDist3);
  # hBkgShape_EventsDist3 = ROOT.TH1D( "hBkgShape_EventsDist", "hBkgShape_EventsDist", 100, 1, n_EventsDist3);
  cnt1_all=0
  cnt2_all=0
  cnt3_all=0
  cnt1_lt1=0
  ...
  with open(ibinMap3) as csvfile:
      binMapping = csv.reader(csvfile, delimiter=',')
      for index,mapmX,mapmY in binMapping:
          cnt3_all+=1
          i = int(index)
          bkgVal =  hbkgIN3.GetBinContent(i)
          # if (125-20<float(mapmY) and float(mapmY)<125+20): continue
          hBkgShape_EventsDist3.Fill(bkgVal)
          # if(bkgVal<1): print("mYx2 binning", mapmX, mapmY)
          if(bkgVal<10):
              cnt3_lt10+=1
              if(bkgVal<5):
                  cnt3_lt5+=1
                  if(bkgVal<1): cnt3_lt1+=1
  # print("Aidan", cnt3_all, hbkgIN3.GetNbinsX())
  ...
  hBkgShape_EventsDist3.Draw("hist same")
  # hError.Draw("samef")
  # hBkg.Draw("same l")
  #xaxis
  ...
  hBkgShape_EventsDist3.GetXaxis().SetRangeUser(0, n_EventsDist3+1)
  ...
  hBkgShape_EventsDist3.GetYaxis().SetRangeUser(0, maxYrange*1.2)
  ...
  hBkgShape_EventsDist3.GetYaxis().SetTitle("Entries/bin")
  hBkgShape_EventsDist3.GetYaxis().SetLabelSize(0.05)
  hBkgShape_EventsDist3.GetYaxis().SetTitleSize(0.05)
  hBkgShape_EventsDist3.GetYaxis().SetTitleOffset(1.1)
  hBkgShape_EventsDist3.GetYaxis().SetTickLength(0.02)
  hBkgShape_EventsDist3.GetXaxis().SetTitle("Num. of Events")
  hBkgShape_EventsDist3.GetXaxis().SetLabelSize(0.05)
  hBkgShape_EventsDist3.GetXaxis().SetTitleSize(0.05)

  hBkgShape_EventsDist1.SetLineWidth(2)
  hBkgShape_EventsDist2.SetLineWidth(2)
  hBkgShape_EventsDist3.SetLineWidth(2)
  hBkgShape_EventsDist1.SetLineColor(ROOT.kRed+2)
  hBkgShape_EventsDist2.SetLineColor(ROOT.kBlue+2)
  hBkgShape_EventsDist3.SetLineColor(ROOT.kGreen+2)
  # hError.SetLineColor(0)

  #### define legend
  legName1 = "Orig. Binning"
  legName2 = "mY bins x2"
  legName3 = "mY bins x2, cut low stat bins"
  leg = TLegend(0.2,0.75,0.45,0.89)
  leg.AddEntry(hBkgShape_EventsDist1, legName1, "l")
  leg.AddEntry(hBkgShape_EventsDist2, legName2, "l")
  leg.AddEntry(hBkgShape_EventsDist3, legName3, "l")
  ...
  ofileMean3.write("{0},{1},{2:.4f}\n".format(mXval, mYval, hBkgShape_EventsDist3.GetMean()))
  ...
  ofilelt1_3.write("{0},{1},{2:.4f}\n".format(mXval, mYval, float(cnt3_lt1)/float(cnt3_all)))
  ...
  ofilelt5_3.write("{0},{1},{2:.4f}\n".format(mXval, mYval, float(cnt3_lt5)/float(cnt3_all)))
  ...
  ofilelt10_3.write("{0},{1},{2:.4f}\n".format(mXval, mYval, float(cnt3_lt10)/float(cnt3_all)))
  odir = "FitDiagnostics/plots_{0}vs{1}vs{2}EventDist".format(tag1, tag2, tag3)
  if (not os.path.exists(odir)):
      os.mkdir(odir)
  odir = "FitDiagnostics/plots_{0}vs{1}vs{2}EventDist/{3}/".format(tag1, tag2, tag3, fittype)
  if (not os.path.exists(odir)):
      os.mkdir(odir)
  odir = "FitDiagnostics/plots_{0}vs{1}vs{2}EventDist/{3}/{4}/".format(tag1, tag2, tag3, fittype,year)
  if (not os.path.exists(odir)):
      os.mkdir(odir)
  # saveName = "{0}fitRatio{1}_{2}.pdf".format(odir, sig, year)
  saveName = "{0}eventDist{1}_{2}.pdf".format(odir, sig, year)
  c1.SaveAs(saveName)
  del c1
...
for afittype in allfittypes:
    for year in years:
        ...
        iodir3 = "/uscms/homes/a/agrummer/nobackup/DiHiggs_v2/CMSSW_10_2_5/src/bbbbAnalysis/VarPlots/rootHists/fullSubmission_2022Nov/{0}DataPlots_2023Jul5_binMYx2_ncMCStats_lowStatsCut_10ev/".format(year)
        with open("{0}/{1}_{3}_{2}.txt".format(odir_EventDist, "mean", year, tag1), "w") as ofileMean1,\
                ...
                open("{0}/{1}_{3}_{2}.txt".format(odir_EventDist, "mean", year, tag3), "w") as ofileMean3,\
                ...
                open("{0}/{1}_{3}_{2}.txt".format(odir_EventDist, "lt1", year, tag3), "w") as ofilelt1_3,\
                ...
                open("{0}/{1}_{3}_{2}.txt".format(odir_EventDist, "lt5", year, tag3), "w") as ofilelt5_3,\
                open("{0}/{1}_{3}_{2}.txt".format(odir_EventDist, "lt10", year, tag1), "w") as ofilelt10_1,\
                open("{0}/{1}_{3}_{2}.txt".format(odir_EventDist, "lt10", year, tag3), "w") as ofilelt10_3,\
            ...
            for i, sig in enumerate(sigs):
                ...
                ibinfile3 = iodir3 + "outPlotter_UnrollLocation_massGroup{0}.txt".format(i)
                ...
                inputFile3 = "/uscms/homes/a/agrummer/nobackup/DiHiggs_v2/CMSSW_10_2_5/src/bbbbAnalysis/VarPlots/rootHists/fullSubmission_2022Nov/{0}DataPlots_{1}/outPlotter_massGroup{2}.root".format(year, tag3, i)
# labels = ["prefit", tag, year, "Mass Group {}".format(0), sig]
                makePlot(labels, ibinfile1, ibinfile2, ibinfile3, inputFile1, inputFile2, inputFile3)
