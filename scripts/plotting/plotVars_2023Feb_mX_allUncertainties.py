...
def rootplot_2samp_ratio( h1, h2, year, region, var, tag, odir, href, h_up, h_down ):
    ...
    # Find the first alternative shape
    for i in range(h3.GetSize()):
        avg_shape = (h_up.GetBinContent(i) + h_down.GetBinContent(i))/2
        hshape1.SetBinContent(i, avg_shape)
    # Find the second alternative shape (in the opposite direction of the first with respect to 3b hist)
    hshape2 = hshape1.Clone("hshape2")
    hshape2.Add(h1, -1) 
    hshape2.Scale(-1)
    hshape2.Add(h1)

    #### normalize the histograms
    # hshape1.Scale(h3.Integral()/hshape1.Integral())
    # hshape2.Scale(h3.Integral()/hshape2.Integral())
    ...
    for i in range(h3.GetSize()):
        ...
        norm_err = h3.GetBinContent(i)*0.03
        if (h3.GetBinContent(i)>0):
            h3.SetBinError(i,h3.GetBinError(i)/h3.GetBinContent(i))
            bin_err_up = math.sqrt(stat_err**2 + shape_err_up**2 + norm_err**2)/h3.GetBinContent(i)
            bin_err_down = math.sqrt(stat_err**2 + shape_err_down**2 + norm_err**2)/h3.GetBinContent(i)
    ...
    #### define legend
    leg = TLegend(0.6,0.72,0.85,0.89)
    leg.AddEntry(h1, "3b data (bkg. model)", "l")
    leg.AddEntry(h2, "4b data (target)", "l")
    leg.AddEntry(h3, "3b data unc. (stat+shape+norm)", "f")
...
#  # data plots
    ...
    #  varname = "H1_b1_kinFit_ptRegressed"
        ...
        rootplot_2samp_ratio( h_3b_weights, h_4b, year, region, varname, "weights", odir, h_3b, h_3b_weights_up, h_3b_weights_down )
##################################################
...
gROOT.SetBatch(True)
# ********************
odir = "VarPlots/2023Feb20_mXmY_shapeUnc/"
