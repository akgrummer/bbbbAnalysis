...
def rootplot_2Dhist(h2d, var1, var2, odir, tag, region, xRange=None, yRange=None, zRange=None):
    ...
    def CreatePlot(h1, tag):
        ...
        if ("sig4b_600_MY_400" in tag):
            plotlabels.DrawLatexNDC(0.3, 0.85, "Signal: mX = 600 GeV, mY = 400 GeV")
        elif ("3b" in tag):
            plotlabels.DrawLatexNDC(0.3, 0.85, "3b data (no BDT weights)")
        if ("Jet1" in tag):
            plotlabels.DrawLatexNDC(0.3, 0.81, "First B-Jet")
        if ("Jet2" in tag):
            plotlabels.DrawLatexNDC(0.3, 0.81, "Second B-Jet")
        if ("Jet3" in tag):
            plotlabels.DrawLatexNDC(0.3, 0.81, "Third B-Jet")
        if ("Jet4" in tag):
            plotlabels.DrawLatexNDC(0.3, 0.81, "Fourth B-Jet")
...
def rootplot_2samp_ratio( h1, h2, year, region, var, tag, odir, href ):
    ...
    KSlabel.DrawTextNDC(0.72, 0.65, ksval)
    KSlabel.DrawTextNDC(0.72, 0.61, ksvalMaxDist)
    KSlabel.DrawTextNDC(0.72, 0.57, ksvalX)
    ...
    #  labelText = "mX = %.0f GeV, mY = %.0f GeV"%(mXval,mYval)
    labelText = ""
    if "VR" in region:    labelText = labelText + "3b Validation Region"
    if "CR" in region:    labelText = labelText + "3b Control Region"
    if "SR" in region:    labelText = labelText + "3b Signal Region"
    plotlabels.DrawLatexNDC(0.60, 0.93, labelText)
    plotlabels.SetTextFont(63)
    plotlabels.SetTextSize(16)
    if "weights" in tag:    
    ...
    plotlabels.DrawLatexNDC(0.15, 0.85, "no kinFit Integral: %0.4f"%(h1area))
    plotlabels.DrawLatexNDC(0.15, 0.81, "kinFit Integral: %0.4f"%(h2area))
    ...
    hErrors.Divide(h1)
    for i in range(h3.GetSize()):
        h3.SetBinContent(i,hErrors.GetBinContent(i))
    #  for i in xrange( b)
    # h4 = h1.Clone("h4")
    # #  print("number of bins, 3b: ", h1.GetSize())
    # #  print("a bin error %.4f"%(h1.GetBinError(50)))
    ...
    h4.SetLineColor(1)
    h4.SetLineWidth(0)
    h4.SetFillColor(ROOT.kAzure+1)

    #### define legend
    leg = TLegend(0.7,0.75,0.95,0.89)
    leg.AddEntry(h1, "no Kin. Fit.", "l")
    leg.AddEntry(h2, "Kin. Fit.", "l")
    leg.AddEntry(h3, "no Kin. Fit. unc.", "le")
    leg.AddEntry(h4, "Kin. Fit. unc.", "f")
    ...
    h4.Draw("e2same") # draw as data points
    h3.Draw("psame") # draw as data points
    ...
    hErrors.GetYaxis().SetRangeUser(0., 2.)
    ...
    hErrors.GetYaxis().SetTitle("kinFit / no kinFit")
    odir = odir + "/" + region
    if not (os.path.exists(odir)): os.makedirs(odir) 
    #  odirpng = odir + "/png"
...
##################################################
    ...
    h_sig_MX_600_MY_400_1  = gDirectory.Get(dir_sig_MX_600_MY_400 +"_"+dir_region+"_"+"FirstBtaggedJet_eta_FirstBtaggedJet_phi")
    h_sig_MX_600_MY_400_2  = gDirectory.Get(dir_sig_MX_600_MY_400 +"_"+dir_region+"_"+"SecondBtaggedJet_eta_SecondBtaggedJet_phi")
    h_sig_MX_600_MY_400_3  = gDirectory.Get(dir_sig_MX_600_MY_400 +"_"+dir_region+"_"+"ThirdBtaggedJet_eta_ThirdBtaggedJet_phi")
    h_sig_MX_600_MY_400_4  = gDirectory.Get(dir_sig_MX_600_MY_400 +"_"+dir_region+"_"+"FourthBtaggedJet_eta_FourthBtaggedJet_phi")

    rootplot_2Dhist(h_sig_MX_600_MY_400_1, "b_eta", "b_phi", odir, "sig4b_600_MY_400_Jet1", region)
    rootplot_2Dhist(h_sig_MX_600_MY_400_2, "b_eta", "b_phi", odir, "sig4b_600_MY_400_Jet2", region)
    rootplot_2Dhist(h_sig_MX_600_MY_400_3, "b_eta", "b_phi", odir, "sig4b_600_MY_400_Jet3", region)
    rootplot_2Dhist(h_sig_MX_600_MY_400_4, "b_eta", "b_phi", odir, "sig4b_600_MY_400_Jet4", region)

    myfile.cd(dir_data_3b+"/"+dir_region)
    h_sig_3b_1 = gDirectory.Get(dir_data_3b +"_"+dir_region+"_"+"FirstBtaggedJet_eta_FirstBtaggedJet_phi")
    h_sig_3b_2 = gDirectory.Get(dir_data_3b +"_"+dir_region+"_"+"SecondBtaggedJet_eta_SecondBtaggedJet_phi")
    h_sig_3b_3 = gDirectory.Get(dir_data_3b +"_"+dir_region+"_"+"ThirdBtaggedJet_eta_ThirdBtaggedJet_phi")
    h_sig_3b_4 = gDirectory.Get(dir_data_3b +"_"+dir_region+"_"+"FourthBtaggedJet_eta_FourthBtaggedJet_phi")
    rootplot_2Dhist(h_sig_3b_1, "b_eta", "b_phi", odir, "3b_Jet1", region)
    rootplot_2Dhist(h_sig_3b_2, "b_eta", "b_phi", odir, "3b_Jet2", region)
    rootplot_2Dhist(h_sig_3b_3, "b_eta", "b_phi", odir, "3b_Jet3", region)
    rootplot_2Dhist(h_sig_3b_4, "b_eta", "b_phi", odir, "3b_Jet4", region)
##################################################
...
#  # data plots
    # myfile.cd(dir_data_3b+"/"+dir_region)
    # h_3b_1 = gDirectory.Get(dir_data_3b+"_"+dir_region+"_HH_m")
    # myfile.cd(dir_data_3b+"/"+dir_region)
    # h_3b_2 = gDirectory.Get(dir_data_3b+"_"+dir_region+"_HH_kinFit_m")
    # rootplot_2samp_ratio( h_3b_1, h_3b_2, year, region, "HH_kinFit_m", "3b", odir, h_3b_1 )
##################################################
...
# iTags = ["2022Jul7_fullBDT_bJetScoreLoose"]
iTags = ["2023Feb13_preapprovalVars2"]
for iTag in iTags:
   for year in years:
