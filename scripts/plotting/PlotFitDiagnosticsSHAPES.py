...
def makePlot(labels,xMin, xMax, globalMaxVal,massMap):
    ...
    if ("pre" in fittype):
        fitLabel = "Pre-fit"
    plotlabels.SetTextFont(63)
    plotlabels.DrawLatexNDC(0.62, 0.83,fitLabel)
    plotlabels.SetTextFont(53)
    plotlabels.DrawLatexNDC(0.62, 0.77,"Mass Group {}".format(massGroup))
...
#       hDataShapeError.SetLineWidth(1)
    ...
    elif ( "fit_b" in fittype ): hpullShape.GetYaxis().SetTitle("pull #[]{#frac{data-bkg}{#sigma_{data}}}")
    ...
    for i in range(xMin,xMax):
        ...
        if lastmY != thismY:
            ...
            p1.cd(); mYlines.DrawLine(i-1,0.05*yMax,i-1,yMax*1.22); p3.cd(); mYlines.DrawLine(i-1,0.95*-5,i-1,0.95*5); p2.cd()
            if (massGroup==4):
    ...
    odir = "FitDiagnostics/SHAPES_1D_{0}_updated/".format(tag)
...
allfittypes= ["prefit", "fit_b", "prefit"]
massMap={}
for year in years:
    locDir=locDirTemplate.format(year, ogTag)
    for i, sig in enumerate(sigs):
        massMap.clear()
        locFile=locDir + locFileTemplate.format(i)
        ...
        for k in range(1, len(BinRanges)):
            ...
            for afittype in allfittypes:
                labels = [afittype, tag, year, i, sig]
