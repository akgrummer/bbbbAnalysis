...
def rootplot_2Dhist(h1, year, odir, tag, descriptionLabel):
    ...
    if("pull" in descriptionLabel): h1.GetZaxis().SetRangeUser(-2.8, 2.8)
    ...
    if ("pull" in descriptionLabel): h1.GetZaxis().SetTitle("pull")
    else: h1.GetZaxis().SetTitle("pull")
    ...
    if("fit_s" in tag):
        fitDir="fit_s"
    if not os.path.isdir("FitDiagnostics/pulls/{0}".format(year)):
        os.mkdir("FitDiagnostics/pulls/{0}".format(year))
    if not os.path.isdir("FitDiagnostics/pulls/{0}/{1}".format(year,fitDir)):
        os.mkdir("FitDiagnostics/pulls/{0}/{1}".format(year, fitDir))
    c1.SaveAs("FitDiagnostics/pulls/{1}/{2}/{0}_{1}.pdf".format(tag, year, fitDir))
...
def makePlotsPerYear(year, pvalFile, labels, inputFile):
  ...
  rootplot_2Dhist(h2dVR_pullShape, year, iodir, tag + "_pullShape_massGroup" + args.massGroup+"_{0}".format(fittype), "{0} pull (shape)".format(fittype))
  rootplot_1Dhist(hpullDistribution, year, iodir, tag + "_pullDist_massGroup" + args.massGroup+"_{0}".format(fittype), "{0} pull (shape)".format(fittype))
  del h2dVR_pull
  del h2dVR_data
  del h2dVR_bkg
  del h2dVR_INbkg
