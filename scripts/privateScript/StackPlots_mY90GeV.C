...
void StackAllDatasets(TVirtualPad *theCanvas, TFile* inputFile, const std::string& selectionAndRegion, const std::string& variableName, const std::string& modelDataset, const std::string& modelName, const std::vector<std::string>& signalDatasetList, const std::string& signalName, const std::string& dataDataset, const std::string& dataName, bool normalizeBinByBin, const std::string& xAxisName, const std::string& yAxisName, const int year, const TString mYlabel, const TString sample)
{
    ...
    std::vector<std::string> datasetList     = {"ggF_Hbb","VBF_Hbb", "WH" ,"ZH"  ,"ZZ"  ,"ttH",  "ttbar", "QCD"   };
    std::vector<std::string> datasetNameList = {"ggF_Hbb","VBF_Hbb", "WH" ,"ZH"  ,"ZZ"  ,"ttH",  "ttbar", "QCD"   };
    // std::vector<EColor>      colorList       = {kRed     , kBlue   , kYellow, kGreen, kViolet, kCyan, kGray, kOrange};
    std::vector<Int_t>      colorList       = { TColor::GetColor("#fdffb6"), TColor::GetColor("#9bf6ff"), TColor::GetColor("#daae66"), TColor::GetColor("#cc00cc"), TColor::GetColor("#16537e"), TColor::GetColor("#caffbf"), TColor::GetColor("#a0c4ff"), TColor::GetColor("#ffc6ff") };

    StackPlot(theCanvas, inputFile, selectionAndRegion, variableName, datasetList, datasetNameList, colorList, modelDataset, modelName, signalDatasetList, signalName, dataDataset, dataName, normalizeBinByBin, xAxisName, yAxisName, year, mYlabel, sample);
...
void StackAllVariables(const TString inputFileName, TString tag, const TString mYlabel, const TString sample, const std::string& selectionAndRegion, int year, const std::string& modelDataset=NO_PLOT, const std::string& modelName="BKG model",  const std::vector<std::string>& signalDatasetList = {}, const std::string& signalName="X{#rightarrow}YH", const bool vsMY=false)
{
    ...
    for(size_t vIt = 0; vIt<variableVector.size(); ++vIt)
        StackAllDatasets(theCanvas->cd(vIt+1), &inputFile, selectionAndRegion, variableVector[vIt], modelDataset, modelName, signalDatasetList, signalName, dataDataset, dataName, normalizeBinByBinVector[vIt], xAxisVector[vIt], yAxisVector[vIt], year, mYlabel, sample);
    TString odir = "VarPlots/BkgComposition/";
    odir=odir+tag+"/";
    std::error_code err;
...
void StackPlots_mY90GeV(TString tag, TString mYlabel, TString sample, TString plotAgainstMY)
{

    // TString tag= "2022Nov14_bJetScoreLoose_shapes_allVars_selectSigs"
    TString idir= "VarPlots/rootHists/fullSubmission_2022Nov/2016DataPlots_";
    TString year2016dir= "2016DataPlots_";
    ...
    StackAllVariables("VarPlots/rootHists/fullSubmission_2022Nov/2016DataPlots_"+tag+"/outPlotter.root", tag, mYlabel, sample,  "selectionbJets_SignalRegion"           , 2016, "data_BTagCSV_dataDriven_kinFit", "Bkg model", {}, "", vsMY);
    StackAllVariables("VarPlots/rootHists/fullSubmission_2022Nov/2016DataPlots_"+tag+"/outPlotter.root", tag, mYlabel, sample, "selectionbJets_ValidationRegionBlinded", 2016, "data_BTagCSV_dataDriven_kinFit", "Bkg model", {}, "", vsMY);
    StackAllVariables("VarPlots/rootHists/fullSubmission_2022Nov/2016DataPlots_"+tag+"/outPlotter.root", tag, mYlabel, sample, "selectionbJets_ControlRegionBlinded", 2016, "data_BTagCSV_dataDriven_kinFit", "Bkg model", {}, "", vsMY);
    // StackAllVariables("DataPlots_fullSubmission_2016_v17/outPlotter.root", "selectionbJets_ValidationRegionBlinded", 2016, "data_BTagCSV_dataDriven_kinFit", "Bkg model", {"sig_NMSSM_bbbb_MX_700_MY_300"}, "X#rightarrowYH (m_{X} = 700 GeV, m_{Y} = 300 GeV)", "data_BTagCSV", "data", vsMY);

    StackAllVariables("VarPlots/rootHists/fullSubmission_2022Nov/2017DataPlots_"+tag+"/outPlotter.root", tag, mYlabel, sample, "selectionbJets_SignalRegion"           , 2017, "data_BTagCSV_dataDriven_kinFit", "Bkg model", {}, "", vsMY);
    StackAllVariables("VarPlots/rootHists/fullSubmission_2022Nov/2017DataPlots_"+tag+"/outPlotter.root", tag, mYlabel, sample, "selectionbJets_ValidationRegionBlinded", 2017, "data_BTagCSV_dataDriven_kinFit", "Bkg model", {}, "", vsMY);
    StackAllVariables("VarPlots/rootHists/fullSubmission_2022Nov/2017DataPlots_"+tag+"/outPlotter.root", tag, mYlabel, sample, "selectionbJets_ControlRegionBlinded", 2017, "data_BTagCSV_dataDriven_kinFit", "Bkg model", {}, "", vsMY);
    ...
    StackAllVariables("VarPlots/rootHists/fullSubmission_2022Nov/2018DataPlots_"+tag+"/outPlotter.root", tag, mYlabel, sample, "selectionbJets_SignalRegion"           , 2018, "data_BTagCSV_dataDriven_kinFit", "Bkg model", {}, "", vsMY);
    StackAllVariables("VarPlots/rootHists/fullSubmission_2022Nov/2018DataPlots_"+tag+"/outPlotter.root", tag, mYlabel, sample, "selectionbJets_ValidationRegionBlinded", 2018, "data_BTagCSV_dataDriven_kinFit", "Bkg model", {}, "", vsMY);
    StackAllVariables("VarPlots/rootHists/fullSubmission_2022Nov/2018DataPlots_"+tag+"/outPlotter.root", tag, mYlabel, sample, "selectionbJets_ControlRegionBlinded", 2018, "data_BTagCSV_dataDriven_kinFit", "Bkg model", {}, "", vsMY);
