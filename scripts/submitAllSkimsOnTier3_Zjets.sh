...
submitSkimsForMassPoint(){
    ...
    submitTag="Zjets_${year}_HT${htRange}2"
    fileName="ZJetsToQQ_HT-${htRange}_TuneCP5_13TeV-madgraphMLM-pythia8"
    ...
    # --append=_MY_60
    # --yMassSelection=60
    # eos root://cmseos.fnal.gov mkdir -p %s' % EOSfilelistBase.replace('root://cmseos.fnal.gov/', '/eos/uscms')
    if [ "${prep}" == "true" ]; then
        # echo "${PATH}"
        echo "run these commands manually..."
        echo "eosmkdir -p /eos/uscms/store/user/${USER}/bbbb_ntuples/${submitTag}/${fileName}/filelist/"
        return 0
    fi
