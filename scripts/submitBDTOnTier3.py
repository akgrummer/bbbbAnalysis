...
jobsDir                = 'BDTgridSubmit/CondorJobs/jobsBDT_' + args.tag
outScriptNameBareProto = 'job_year{0}_seed{1}.sh'
...
def writeScript(year, seed):
    outScriptName  = outScriptNameProto.format(year,seed)
    ...
    #  applyBDTcommand = 'python mlskim_NMSSM_XYH_bbbb/ApplyBackgroundModel.py --dir BDToutput/fullSubmission_{0}_v27_BDTweights_MassWindow_2022Apr18_seed{1} --mode w --seed {1}'
...
#  #  seeds = 2020
for year in years:
    for seed in range(2020,3020):
#  year=2016
#  seed=2020
        writeScript(year, seed)
        command = "%s/scripts/t3submit %s" % (bbbbWorkDir, outScriptNameProto.format(year,seed))
        if os.system(command) != 0:
            print "... Not able to execute command \"", command, "\", exit"
            sys.exit()
