...
void ProduceAllTriggerEfficiencyInAFile2016(std::vector<std::tuple<std::shared_ptr<TGraphAsymmErrors>,std::shared_ptr<TH1F> > >& theOutputResultVector, std::string inputFileName, std::string datasetName, float expectedNumberOfEvents, Color_t theColor, bool useTTbarCut)
{
    ...
    theEfficiencyEvaluator.addTrigger(triggerName, filterCutDouble90Quad30, "caloJetSum"                  , normalizationCutDouble90Quad30, "L1filterHT; #sum p_{T} [GeV]; online efficiency"                 ,75, 100,1500, theColor);
    ...
    customBinning.clear();
    customBinCreator(customBinning, 20., 100., 4.,  180., 16.,  240., 30.,  300., 60.);
    ...
    theEfficiencyEvaluator.addTrigger(triggerName, filterCutDouble90Quad30, "jetSecondHighestPt_pt"         , normalizationCutDouble90Quad30, "DoubleCentralJet90; p_{T}^{2} [GeV]; online efficiency"         ,60, 20 , 500, theColor);
    ...
    if(datasetName == "NMSSM_XYH_bbbb") filterCutDouble90Quad30 = normalizationCutDouble90Quad30 + "&& jetFirstHighestDeepFlavB_triggerFlag>=1";
    customBinning.clear();
    if(isSingleMuon) customBinCreator(customBinning, 0., 0.8, 0.05, 1., 0.04);
    else             customBinCreator(customBinning, 0., 0.5, 0.10, 0.9, 0.08,  1., 0.02);
    ...
    customBinning.clear();
    customBinCreator(customBinning, 20., 100., 5.,  150., 25.,  190., 40.,  290., 100.);
    ...
    customBinning.clear();
    customBinCreator(customBinning, 20., 70., 50.,  200., 10.,  300., 20.,  500., 50.);
    ...
    theEfficiencyEvaluator.addTrigger(triggerName, filterCutQuad45        , "caloJetSum"                  , normalizationCutQuad45        , "L1filterHT; #sum p_{T} [GeV]; online efficiency"                 ,75, 100,1500, theColor);
    
    normalizationCutQuad45 = filterCutQuad45;
    filterCutQuad45 = normalizationCutQuad45 + "&& QuadCentralJet45>=4";
    customBinning.clear();
    customBinCreator(customBinning, 20., 60., 5., 100., 10., 160., 30.,  280., 60.);
    ...
    if(datasetName == "NMSSM_XYH_bbbb") filterCutQuad45 = normalizationCutQuad45 + "&& jetFirstHighestDeepFlavB_triggerFlag>=1";
    customBinning.clear();
    if(isSingleMuon) customBinCreator(customBinning, 0., 0.8, 0.05, 1., 0.04);
    else             customBinCreator(customBinning, 0., 0.5, 0.10, 0.9, 0.08,  1., 0.02);
    ...
    theEfficiencyEvaluator.addTrigger(triggerName, filterCutAnd           , "caloJetSum"                  , normalizationCutAnd           , "L1filterQuad45HT; #sum p_{T} [GeV]; online efficiency"           ,20, 100,1500, theColor);
    
    normalizationCutAnd = filterCutAnd;
    filterCutAnd = normalizationCutAnd + "&& QuadCentralJet45>=4";
    customBinning.clear();
    customBinCreator(customBinning, 20., 40., 20.,  100., 10.,  130., 30.,  160., 60.,  300., 140.);
    ...
    if(datasetName == "NMSSM_XYH_bbbb") filterCutAnd = normalizationCutAnd + "&& jetFirstHighestDeepFlavB_triggerFlag>=1";
    customBinning.clear();
    if(isSingleMuon) customBinCreator(customBinning, 0., 0.8, 0.05, 1., 0.04);
    ...
    customBinning.clear();
    customBinCreator(customBinning, 20., 40., 20.,  100., 10.,  130., 30.,  190., 60.,  310., 120.);
...
void ProduceAllTriggerEfficienciesFiles2016(std::string singleMuonInputFileName, std::string ttbarInputFileName, std::string xyhInputSignal, std::string outputFileName, bool useTTbarCut)
{
...
void ProduceAllTriggerEfficiencies2016()
{
    ...
    std::thread theMatchedTriggerThreadTTbarCut  (ProduceAllTriggerEfficienciesFiles2016, "SingleMuon_Data_forTrigger_2016.root"  , "TTbar_MC_forTrigger_2016.root"  ,  "NMSSM_forTrigger_2016.root",  "TriggerEfficiencies_2016_TTBarCut_Rebin.root", true );
...
void ProduceAllTriggerEfficiencies2017()
{
    ...
    std::thread theMatchedTriggerThread          (ProduceAllTriggerEfficienciesFiles2017, "SingleMuon_Data_forTrigger_2017.root"  , "TTbar_MC_forTrigger_2017.root"  , "NMSSM_forTrigger_2017.root",   "TriggerEfficiencies_2017_TTBarCut_Rebin.root"           , true);
...
void ProduceAllTriggerEfficiencies2018()
{
    ...
    std::thread theMatchedTriggerThread          (ProduceAllTriggerEfficienciesFiles2018, "SingleMuon_Data_forTrigger_2018.root"  , "TTbar_MC_forTrigger_2018.root"  , "NMSSM_forTrigger_2018.root",   "TriggerEfficiencies_2018_TTBarCut_Rebin.root"           , true);
