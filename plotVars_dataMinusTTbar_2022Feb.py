...
def rootplot_2samp_ratio( h1, h2, year, region, var, tag, odir, href ):
    ...
    plotlabels.SetTextFont(63)
    plotlabels.SetTextSize(20)
    plotlabels.DrawLatexNDC(0.35, 0.78, "Data - TTBAR")
...
def makeplotsForRegion(dir_region, region, odir, year):
    ...
    idir_ttbar = "VarPlots/rootHists/%sDataPlots_2022Mar4_fullBDT_withTTBAR_MassWindow_3b_sig"%(year) # outPlotter.root is the same for CR and VR (normal binning)
    #  odir = odir + "MassWindow/"
    #  odir = odir + "All/"
    odir = odir + year
    myfile = TFile.Open(idir + "/outPlotter.root")
    myfile_ttbar = TFile.Open(idir_ttbar + "/outPlotter.root")
...
##################################################
    ...
    #  varname = "H1_b1_kinFit_ptRegressed"
        ...
        h_4b = gDirectory.Get(dir_data_4b+"_"+dir_region+"_"+varname)
        # ttbar
        myfile_ttbar.cd(dir_ttbar_3b + "/" + dir_region)
        h_3b_ttbar = gDirectory.Get(dir_ttbar_3b+"_"+dir_region+"_"+varname)
        #  print("Integral: %0.4f"% (h_3b_weights.Integral("width")))
        #  print("Entries: %0.4f"% (h_3b_weights.GetEntries()))
        myfile_ttbar.cd(dir_ttbar_3b_weights+"/"+dir_region)
        h_3b_weights_ttbar = gDirectory.Get(dir_ttbar_3b_weights+"_"+dir_region+"_"+varname)
        myfile_ttbar.cd(dir_ttbar_4b+"/"+dir_region)
        h_4b_ttbar = gDirectory.Get(dir_ttbar_4b+"_"+dir_region+"_"+varname)
        #  print("Integral: %0.4f"% (h_3b_weights.Integral("width")))
        #  print("Entries: %0.4f"% (h_3b_weights.GetEntries()))
        h_3b.Add(h_3b_ttbar,-1);
        h_3b_weights.Add(h_3b_weights_ttbar,-1);
        h_4b.Add(h_4b_ttbar,-1);
        rootplot_2samp_ratio( h_3b        , h_4b, year, region, varname, "orig"   , odir, h_3b_weights )
        rootplot_2samp_ratio( h_3b_weights, h_4b, year, region, varname, "weights", odir, h_3b         )
##################################################
...
odir = "VarPlots/DataMinusTTbar2022Mar9/"
