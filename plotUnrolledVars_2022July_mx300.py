...
from ROOT import kRed, kGreen, kMagenta, kBlack 
...
def rootplot_2samp_ratio( h1, h2, year, region, var, tag, label, regionLabel, odir, hsig60, hsig150,yMaxVal,massGroup):
    #### get the histograms:
    h1 = h1.Clone("h1copy")
    h2 = h2.Clone("h2copy")
    hsig60 = hsig60.Clone("hsigcopy")
    hsig150 = hsig150.Clone("hsigcopy")
    ...
    #  if("_sig" in tag):
    hsig60.Scale(h2.Integral()/hsig60.Integral())
    hsig60.Scale(0.15)
    hsig150.Scale(h2.Integral()/hsig150.Integral())
    hsig150.Scale(0.15)
    ...
    h2.SetLineColor(kBlue+2)
    h2.SetLineWidth(2)
    hsig60.SetLineColor(kGreen+2)
    hsig150.SetLineColor(kMagenta+2)
    if("_sig" in tag): hsig.SetLineColor(kGreen+2)
    if("_ttbar" in tag): hsig.SetLineColor(kBlack)
    hsig60.SetLineWidth(2)
    hsig150.SetLineWidth(2)
    h1.Draw("hist")
    h2.Draw("hist same")
    hsig60.Draw("hist same")
    hsig150.Draw("hist same")
    #xaxis
    ...
    h2.GetYaxis().SetTitleOffset(1.1)
    h2.GetYaxis().SetTickLength(0.02)
    hsig60.GetYaxis().SetTitle(varInfo[var]['YaxisTitle'])
    hsig60.GetYaxis().SetLabelSize(0.05)
    hsig60.GetYaxis().SetTitleSize(0.05)
    hsig60.GetYaxis().SetTitleOffset(1.1)
    hsig60.GetYaxis().SetTickLength(0.02)
    hsig150.GetYaxis().SetTitle(varInfo[var]['YaxisTitle'])
    hsig150.GetYaxis().SetLabelSize(0.05)
    hsig150.GetYaxis().SetTitleSize(0.05)
    hsig150.GetYaxis().SetTitleOffset(1.1)
    hsig150.GetYaxis().SetTickLength(0.02)
    ...
    #### define legend
    leg = TLegend(0.68,0.72,0.95,0.89)
    leg.AddEntry(h1, "3b data (bkg. model)", "l")
    leg.AddEntry(h2, "4b data (target)", "l")
    leg.AddEntry(hsig60, "mX=300,mY=60 (15%3b)", "l")
    leg.AddEntry(hsig150, "mX=300,mY=150 (15%3b)", "l")
    #  if ("_sig" in tag): leg.AddEntry(hsig, "Signal", "l")
    #  if ("_ttbar" in tag): leg.AddEntry(hsig, "ttbar", "l")
    #  leg.AddEntry(h1, "3b ttbar", "l")
    #  leg.AddEntry(h2, "4b ttbar", "l")
...
def compareUnrolled(dir_region, region, year, odir,tag, label, massGroup,yMaxVal, ySigmaMaxVal):
    ...
    #  varname = "_HH_kinFit_m_H2_m"
    # dir_sig = "sig_NMSSM_bbbb_MX_600_MY_400" # signal
    dir_sig60 = "sig_NMSSM_bbbb_MX_300_MY_60" # signal
    dir_sig150 = "sig_NMSSM_bbbb_MX_300_MY_150" # signal
    dir_sig_3b_weights = "sig_NMSSM_bbbb_MX_600_MY_400_3bScaled"
...
#get histograms
    ...
    ifile.cd(dir_sig60+"/"+dir_region)
    hsig60 = gDirectory.Get(dir_sig60+"_"+dir_region+"_"+histname)
    ifile.cd(dir_sig150+"/"+dir_region)
    hsig150 = gDirectory.Get(dir_sig150+"_"+dir_region+"_"+histname)
    ...
    rootplot_2samp_ratio( h_3b_weights, h_4b,  year, region, varname, tag, label, regionLabel, odir, hsig60, hsig150,yMaxVal,massGroup)
...
iTags = {
        ...
        "2022Jul7_fullBDT_bJetScoreLoose_mx300unrollForPlotting":  "full mass plane; depth 4, leafs=300",
...
for year in years:
    for region in regions:
        ...
        for massGroup in range(1):
                yMaxVal=0
                ySigmaMaxVal=0
                for tag, plotlabel in iTags.items():
                    yMaxVal,ySigmaMaxVal = compareUnrolled(region[1], region[0], year, odir, tag,plotlabel,massGroup,yMaxVal,ySigmaMaxVal)
