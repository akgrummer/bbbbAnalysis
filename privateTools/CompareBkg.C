...
float gRatioLowLimit  = 0.;
float gRatioHighLimit = 2.;
...
void plotComparison(int year, std::string variable, std::pair<std::string, std::tuple<std::string, int, float, float>> cut, bool useWeight)
{
    ...
    std::string canvasName = "PlotComparison_year_" + std::to_string(year) + "_" + cut.first + (useWeight ? "_AfterBDT" : "_BeforeBDT");
    ...
    RatioPlot(theCanvasPlot, variable4Btag, {variable3Btag}, {kRed}, normalize, -1, std::get<2>(cut.second), std::get<3>(cut.second), 1, "Distance from Diagonal [GeV]", "events", title, "4b-tag", {std::string(bkgName)});
...
void plotAllDiagonalComparisons(int year, bool divideInRegions, bool useWeight)
{
    std::string variable = "distanceFromDiagonal";
    ...
    else xMassCutList["full"] = std::make_tuple<std::string, int, float, float>("", 150, -100., 2000.); //"HH_kinFit_m > 1000.  && HH_kinFit_m < 2000.";
...
void plotAllComparisons(bool divideInRegions = false, bool useWeight = true)
{
    ...
    for(const auto year : yearList) plotAllDiagonalComparisons(year, divideInRegions, useWeight);
    gROOT->SetBatch(false);

}


void plotMyComparisons(int year, bool divideInRegions, bool useWeight)
{
    std::string variable = "H2_m";

    std::pair<std::string, std::tuple<std::string, int, float, float>> xMassCut {"mY_400_600", std::make_tuple<std::string, int, float, float>("HH_kinFit_m > 400 && HH_kinFit_m < 600", 150, 0., 500.)};

    plotComparison(year, variable, xMassCut, useWeight);
}


void plotAllMyComparisons(bool divideInRegions = false, bool useWeight = true)
{
    gROOT->SetBatch(true);
    triggerSelectionMap[2016] = "(( (HLT_DoubleJet90_Double30_TripleBTagCSV_p087_Fired==1 && HLT_DoubleJet90_Double30_TripleBTagCSV_p087_ObjectMatched>0) || (HLT_QuadJet45_TripleBTagCSV_p087_Fired==1 && HLT_QuadJet45_TripleBTagCSV_p087_ObjectMatched > 0) ))";
    triggerSelectionMap[2017] = "(( (HLT_PFHT300PT30_QuadPFJet_75_60_45_40_TriplePFBTagCSV_3p0_Fired==1 && HLT_PFHT300PT30_QuadPFJet_75_60_45_40_TriplePFBTagCSV_3p0_ObjectMatched>0) || (HLT_HT300PT30_QuadJet_75_60_45_40_TripeCSV_p07_Fired==1 && HLT_HT300PT30_QuadJet_75_60_45_40_TripeCSV_p07_ObjectMatched > 0) ))";
    ...
    for(const auto year : yearList) plotMyComparisons(year, divideInRegions, useWeight);
    gROOT->SetBatch(false);
