...
float getVariableAndFit(TVirtualPad *theCanvas, std::string fileName, std::string variable, int year, float xMin, float xMax)
{
    gROOT->SetBatch(true);
    ...
    return halfWidthHalfMaximum;
    
}

float plotVariableResolution(std::string variable, int year, int mX, int mY)
{
    // gROOT->SetBatch(true);
    ...
    float halfWidthHalfMaximum = getVariableAndFit(theCanvasKinFitImpact,getFileName({mX,mY}), variable, year, xMin, xMax);
...
void PlotAllResolutionsVariable(TFile& theOutputFile, std::string variable, int year)
{
    gROOT->SetBatch(true);
    ...
    std::string histogramName  = "ResolutionPlot_" + variable + "_" + std::to_string(year);
    TH2F* theResolutionPlot = new TH2F(histogramName.c_str(), histogramTitle.c_str(), xBinning.size()-1, xBinning.data(), yBinning.size()-1, yBinning.data());
    theResolutionPlot->GetXaxis()->SetTitle("m_{X} [GeV]");
    theResolutionPlot->GetYaxis()->SetTitle("m_{Y} [GeV]");
    theResolutionPlot->GetYaxis()->SetTitleOffset(1.2);
    theResolutionPlot->SetStats(false);

    for(auto& point : pointList)
    {
        float halfWidthHalfMaximum = plotVariableResolution(variable, year, point.first, point.second);
        float denominator = 1.;
        if(variable == "H1_m" || variable == "H1_kinFit_m")
        {
            denominator = 1.;
        ...
        if(variable == "HH_m" || variable == "HH_kinFit_m")
        {
            denominator = point.first;
        }

        theResolutionPlot->Fill(point.first, point.second, halfWidthHalfMaximum/denominator*100.);
    }

    std::string canvasName = "ResolutionPlot_" + variable + "_" + std::to_string(year);
    TCanvas *theCanvasResolution = new TCanvas(canvasName.c_str(), canvasName.c_str(), 1400, 1000);
    theResolutionPlot->Draw("colz");
    theCanvasResolution->SaveAs((std::string(theCanvasResolution->GetName()) + ".png").c_str());
    theCanvasResolution->Write();
    theOutputFile.WriteObject(theCanvasResolution, theCanvasResolution->GetName());
    gROOT->SetBatch(false);
}

void PlotAllResolutions()
{
    TFile theOutputFile("ResolutionPlot.root", "RECREATE");
    ...
    for(auto year : yearList)
    {
        ...
        {
            PlotAllResolutionsVariable(theOutputFile, variable, year);
        }
    }
    theOutputFile.Close();
}
