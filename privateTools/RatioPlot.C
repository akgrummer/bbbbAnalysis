...
void RatioAllVariables(std::string canvasName, std::string referenceFileName, std::string referenceDatasetName , std::string referenceCutName, std::string targetFileName, std::string targetDatasetName , std::string targetCutName, bool normalize, std::string region = "HMR", bool useKinFitVariables = false)
{
    ...
    RatioPlotFromFile(theCanvas->cd(1),referenceFileName ,referenceHistPrototype + "_H1" + append + "_pt"                       , {targetFileName} , {targetHistPrototype + "_H1" + append + "_pt"                      }, {kRed} , normalize, scaleValue,     0,   600, 4, "pT_{Hreco" + append + "} [GeV]","events","4b-tag",{legEntry});
    scaleValue = -1;
    ...
    scaleValue = -1;
    RatioPlotFromFile(theCanvas->cd(2),referenceFileName ,referenceHistPrototype + "_H1" + append + "_eta"                      , {targetFileName} , {targetHistPrototype + "_H1" + append + "_eta"                     }, {kRed} , normalize, scaleValue,    -4,     4, 2, "#eta_{Hreco" + append + "}","events","4b-tag",{legEntry});
    scaleValue = -1;
    ...
    scaleValue = -1;
    RatioPlotFromFile(theCanvas->cd(6),referenceFileName ,referenceHistPrototype + "_HH" + append + "_m"                        , {targetFileName} , {targetHistPrototype + "_HH" + append + "_m"                       }, {kRed} , normalize, scaleValue,   250,  2000, 4, "m_{Xreco} [GeV]","events/GeV","4b-tag",{legEntry},true);
    // RatioPlotFromFile(theCanvas->cd(9),referenceFileName ,referenceHistPrototype + "_HH_m_H2_m_Rebinned_Unrolled" , {targetFileName} , {targetHistPrototype + "_HH_m_H2_m_Rebinned_Unrolled"}, {kRed} , normalize, scaleValue, minUnroll_m, maxUnroll_m, rebinUnroll_m, "m_{Xreco}*m_{Yreco}");
    // scaleValue = -1;
    ...
    theSecondCanvas->DivideSquare(3,0.005,0.005);
...
void CompareShapes(std::string canvasName, std::string referenceFileName, std::string referenceDatasetName , std::string referenceCutName, std::string targetFileName, std::string targetDatasetName , std::string targetCutName, bool normalize, std::string region = "HMR", bool useKinFitVariables = false)
{
    ...
    std::string append = useKinFitVariables ? "_kinFit" : "";
    std::string referenceHistPrototype = referenceDatasetName +  "/" + referenceCutName + "/" + referenceDatasetName +  "_" + referenceCutName;
    std::string variationUp   = targetDatasetName + "_upRefined"  ;
    std::string variationDown = targetDatasetName + "_downRefined";
    ...
    std::string plotName = referenceHistPrototype + "_HH" + append + "_m_H2_m_Rebinned_Unrolled";
    TFile referenceFile(referenceFileName.data());
    TH1D *referenceHistogram = (TH1D*)referenceFile.Get(plotName.c_str());
    if(referenceHistogram == NULL)
    {
        std::cerr<<"referenceHistogram " << plotName << " does not exist\n";
        return;
    }
    referenceHistogram->SetDirectory(0);
    referenceFile.Close();
    int maxBin = referenceHistogram->GetNbinsX();
...
{
    gROOT->SetBatch(true);
    std::string canvasName = "BackgroundSculpting_offShell_mH" + std::to_string(year);
...
void CompareAllShapesPerGroup(int version=0, bool useKinFitVariables=true, std::string dataDrivenDatasetName="data_BTagCSV_dataDriven_kinFit")
{
    ...
    for(const auto& massGroup : massGroupList)
    {
        CompareAllShapesPerGroup(version, massGroup, useKinFitVariables, dataDrivenDatasetName);
    }
}
