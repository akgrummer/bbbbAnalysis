...
def rootplot_2samp_ratio( h1, h2, year, region, var, tag, odir, href , yAxisRange, ratioRange):
    #### get the histograms:
    h1 = h1.Clone("h1copy")
    h2 = h2.Clone("h2copy")
    ...
    #yaxis
    h1.GetYaxis().SetTitle(varInfo[var]['ZaxisTitle'])
    h1.GetYaxis().SetLabelSize(0.05)
    h1.GetYaxis().SetTitleSize(0.05)
    h1.GetYaxis().SetTitleOffset(1.1)
    h1.GetYaxis().SetTickLength(0.02)
    h2.GetYaxis().SetTitle(varInfo[var]['ZaxisTitle'])
    ...
    h1.GetYaxis().SetRangeUser(yAxisRange[0], yAxisRange[1])
    ...
    #  labelText = "mX = %.0f GeV, mY = %.0f GeV"%(mXval,mYval)
    labelText = ""
    if "3b" in region:    labelText = labelText + "3b Data"
    if "4b" in region:    labelText = labelText + "4b Data"
    ...
    plotlabels.DrawLatexNDC(0.15, 0.85, "Nominal Integral: %0.4f"%(h1area))
    plotlabels.DrawLatexNDC(0.15, 0.81, "UL Integral: %0.4f"%(h2area))
    ...
    h3.GetYaxis().SetRangeUser(ratioRange[0],ratioRange[1])
    h4.GetYaxis().SetRangeUser(ratioRange[0],ratioRange[1])
...
#  odir = "VarPlots/ULplots2022Mar11/"
...
#  nvbins = 69
#  vbins = np.array([36, 45, 51, 57, 62, 66, 70, 74, 78, 82, 86, 90, 94, 98, 102, 106, 110, 114, 122, 132, 140, 148, 156, 164, 172, 180, 188, 196, 204, 212, 228, 244, 260, 276, 292, 308, 324, 340, 356, 372, 388, 412, 444, 476, 508, 540, 572, 604, 636, 668, 700, 732, 764, 828, 892, 956, 1020, 1084, 1148, 1212, 1276, 1340, 1404, 1468, 1564, 1692, 1820, 1948, 2076, 2204])

def runFunction(var, ifile, ifile2, selections, regionSel, year, region, odir, yAxisRange, ratioRange):
    bbbbTreeNom= ifile.Get("bbbbTree")
    bbbbTreeNom.Draw(var+">>+hNom(50,0,3000)",selections+regionSel)
    hNom = gDirectory.Get("hNom")

    bbbbTreeUL = ifile2.Get("bbbbTree")
    bbbbTreeUL.Draw(var+">>+hUL(50,0,3000)",selections+regionSel)
    hUL = gDirectory.Get("hUL")
    rootplot_2samp_ratio( hNom, hUL, year, region, var, "orig"   , odir+year, hNom, yAxisRange, ratioRange)



ifile2016Nom = TFile.Open("/uscms/home/fravera/nobackup/DiHiggs_v2/CMSSW_10_2_5/src/bbbbAnalysis/Nominalsample_2016.root")
ifile2016UL = TFile.Open("/uscms/home/fravera/nobackup/DiHiggs_v2/CMSSW_10_2_5/src/bbbbAnalysis/ULsample_2016.root")

ifile2017Nom = TFile.Open("/uscms/home/fravera/nobackup/DiHiggs_v2/CMSSW_10_2_5/src/bbbbAnalysis/Nominalsample_2017.root")
ifile2017UL = TFile.Open("/uscms/home/fravera/nobackup/DiHiggs_v2/CMSSW_10_2_5/src/bbbbAnalysis/ULsample_2017.root")

ifile2018Nom = TFile.Open("/uscms/home/fravera/nobackup/DiHiggs_v2/CMSSW_10_2_5/src/bbbbAnalysis/Nominalsample_2018.root")
ifile2018UL = TFile.Open("/uscms/home/fravera/nobackup/DiHiggs_v2/CMSSW_10_2_5/src/bbbbAnalysis/ULsample_2018.root")

selections2016 = "H1_b1_pt > 30 && H1_b2_pt > 30 && H2_b1_pt > 30 && H2_b2_pt > 30 && H1_b1_ptRegressed > 30 && H1_b2_ptRegressed > 30 && H2_b1_ptRegressed > 30 && H2_b2_ptRegressed > 30 && H1_b1_eta > -2.4 && H1_b1_eta < 2.4 && H1_b2_eta > -2.4 && H1_b2_eta < 2.4 && H2_b1_eta > -2.4 && H2_b1_eta < 2.4 && H2_b2_eta > -2.4 && H2_b2_eta < 2.4 && IsolatedElectron_pt<15 && IsolatedMuon_pt<10 && ( (HLT_DoubleJet90_Double30_TripleBTagCSV_p087_Fired==1 && HLT_DoubleJet90_Double30_TripleBTagCSV_p087_ObjectMatched>0) || (HLT_QuadJet45_TripleBTagCSV_p087_Fired==1 && HLT_QuadJet45_TripleBTagCSV_p087_ObjectMatched > 0) )"
selections2017 = "H1_b1_pt > 40 && H1_b2_pt > 40 && H2_b1_pt > 40 && H2_b2_pt > 40 && H1_b1_ptRegressed > 40 && H1_b2_ptRegressed > 40 && H2_b1_ptRegressed > 40 && H2_b2_ptRegressed > 40 && H1_b1_eta > -2.5 && H1_b1_eta < 2.5 && H1_b2_eta > -2.5 && H1_b2_eta < 2.5 && H2_b1_eta > -2.5 && H2_b1_eta < 2.5 && H2_b2_eta > -2.5 && H2_b2_eta < 2.5 && IsolatedElectron_pt<15 && IsolatedMuon_pt<10 && ( (HLT_PFHT300PT30_QuadPFJet_75_60_45_40_TriplePFBTagCSV_3p0_Fired==1 && HLT_PFHT300PT30_QuadPFJet_75_60_45_40_TriplePFBTagCSV_3p0_ObjectMatched>0) || (HLT_HT300PT30_QuadJet_75_60_45_40_TripeCSV_p07_Fired==1 && HLT_HT300PT30_QuadJet_75_60_45_40_TripeCSV_p07_ObjectMatched > 0) )"
selections2018 = "H1_b1_pt > 40 && H1_b2_pt > 40 && H2_b1_pt > 40 && H2_b2_pt > 40 && H1_b1_ptRegressed > 40 && H1_b2_ptRegressed > 40 && H2_b1_ptRegressed > 40 && H2_b2_ptRegressed > 30 && H1_b1_eta > -2.5 && H1_b1_eta < 2.5 && H1_b2_eta > -2.5 && H1_b2_eta < 2.5 && H2_b1_eta > -2.5 && H2_b1_eta < 2.5 && H2_b2_eta > -2.5 && H2_b2_eta < 2.5 && IsolatedElectron_pt<15 && IsolatedMuon_pt<10 && (HLT_PFHT330PT30_QuadPFJet_75_60_45_40_TriplePFBTagDeepCSV_4p5_Fired==1 && HLT_PFHT330PT30_QuadPFJet_75_60_45_40_TriplePFBTagDeepCSV_4p5_ObjectMatched>0)"

selection3b = "&& (NbJets == 3) && (H1_m >65) && (H1_m < 185)"
selection4b = "&& (NbJets >= 4) && (((H1_m >65)&&(H1_m <105)) || ((H1_m > 145)&&(H1_m < 185)))"
...
runFunction(varlist[0], ifile2016Nom, ifile2016UL, selections2016, selection3b, "2016", "3b", odir, [ 200, 2e6], [0.5,1.5])
runFunction(varlist[1], ifile2016Nom, ifile2016UL, selections2016, selection3b, "2016", "3b", odir, [ 200, 2e6], [0.,2.])
runFunction(varlist[0], ifile2016Nom, ifile2016UL, selections2016, selection4b, "2016", "4b", odir, [ 1, 2e5], [0.5,1.5])
runFunction(varlist[1], ifile2016Nom, ifile2016UL, selections2016, selection4b, "2016", "4b", odir, [ 1, 2e5], [0.,2.])

runFunction(varlist[0], ifile2017Nom, ifile2017UL, selections2017, selection3b, "2017", "3b", odir, [ 200, 2e6], [0.8,1.2])
runFunction(varlist[1], ifile2017Nom, ifile2017UL, selections2017, selection3b, "2017", "3b", odir, [ 200, 2e6], [0.8,1.2])
runFunction(varlist[0], ifile2017Nom, ifile2017UL, selections2017, selection4b, "2017", "4b", odir, [ 1, 2e5], [0.8,1.2])
runFunction(varlist[1], ifile2017Nom, ifile2017UL, selections2017, selection4b, "2017", "4b", odir, [ 1, 2e5], [0.8,1.2])

runFunction(varlist[0], ifile2018Nom, ifile2018UL, selections2018, selection3b, "2018", "3b", odir, [ 200, 2e6], [0.8,1.2])
runFunction(varlist[1], ifile2018Nom, ifile2018UL, selections2018, selection3b, "2018", "3b", odir, [ 200, 2e6], [0.8,1.2])
runFunction(varlist[0], ifile2018Nom, ifile2018UL, selections2018, selection4b, "2018", "4b", odir, [ 1, 2e5], [0.8,1.2])
runFunction(varlist[1], ifile2018Nom, ifile2018UL, selections2018, selection4b, "2018", "4b", odir, [ 1, 2e5], [0.8,1.2])
