...
parser.add_argument('--input3'    , dest = 'input3'    , help = 'input file' , required = True)
...
inputFile3 = TFile(args.input3, "READ")
...
for xMass in xMassList:
    ...
    inputGraph3 = "Limits_{0}/Option_{1}/CentralLimit_{0}_{1}_massX_{2}".format(args.year, append, xMass)
    theGraph3 = inputFile3.Get(inputGraph3)
    theGraph3.SetLineColor(ROOT.kGreen+2)
    theGraph3.SetLineWidth(1)


    theGraph1.Draw("apl")
    theGraph2.Draw("plsame")
    theGraph3.Draw("plsame")
    ...
    theLegend.SetHeader("Median expected limit (new non-closure)")
    theLegend.AddEntry(theGraph1, "Nominal bins", "lp")
    theLegend.AddEntry(theGraph2, "bins mY x2, cut bins < 5 events", "lp")
    theLegend.AddEntry(theGraph3, "bins mY x2, cut bins < 10 events", "lp")

    theLegend.Draw("same")
    theCanvas.SaveAs("ThreeLimits{0}_{1}_mX_{2}.pdf".format(args.year, append, xMass))
    del theCanvas
