...
int main(int argc, char** argv)
{
    ...
    // std::vector<std::string> minLimitOptions      {"statOnly"};
    std::vector<std::string> minLimitOptions      {"statOnly", "syst"};
    // std::vector<std::string> minLimitOptions      {"statOnly", "freezeBKGnorm"};
    ...
    std::map<std::string, std::map<float, std::tuple<TGraph*, TGraphAsymmErrors*, TGraphAsymmErrors*, TGraph*>>> mapOfLimitsVsY;
    ...
    for(const auto & year : yearList)
    {
        ...
        for(const auto & option : limitOptions)
        {
            ...
            for(const auto yMass : yMassList)
            {
                ...
                std::get<3>(the1Dmap[yMass])->GetXaxis()->SetTitle("m_{X} [GeV]");
                std::get<3>(the1Dmap[yMass])->SetMaximum(10000);
                std::get<3>(the1Dmap[yMass])->SetMinimum(1);
                std::get<3>(the1Dmap[yMass])->SetNameTitle(("ObservedLimit_" + year + "_" + option + "_massY_" + std::to_string(int(yMass))).c_str(), ("Observed Limit - " + year + " " + option + " m_{Y} = " + std::to_string(int(yMass))).c_str());
                std::get<3>(the1Dmap[yMass])->GetXaxis()->SetTitle("m_{X} [GeV]");
                std::get<3>(the1Dmap[yMass])->SetMaximum(10000);
                std::get<3>(the1Dmap[yMass])->SetMinimum(1);
            ...
            if(option == "syst" || option=="freezeBKGnorm")
            {
                for(const auto xMass : xMassList)
                {
                    mapOfLimitsVsY[year][xMass] = std::make_tuple(new TGraph(), new TGraphAsymmErrors(), new TGraphAsymmErrors(), new TGraph());
                    std::get<0>(mapOfLimitsVsY[year][xMass])->SetNameTitle(("CentralLimit_" + year + "_" + option + "_massX_" + std::to_string(int(xMass))).c_str(), ("Central Limit - " + year + " " + option + " m_{Y} = " + std::to_string(int(xMass))).c_str());
                    std::get<0>(mapOfLimitsVsY[year][xMass])->GetXaxis()->SetTitle("m_{Y} [GeV]");
                    std::get<0>(mapOfLimitsVsY[year][xMass])->SetMaximum(10000);
                    std::get<0>(mapOfLimitsVsY[year][xMass])->SetMinimum(1);
                    std::get<0>(mapOfLimitsVsY[year][xMass])->GetYaxis()->SetTitle("#sigma(pp #rightarrow X) #times BR(Y(b#bar{b}) H(b#bar{b})) [fb]");
                    std::get<1>(mapOfLimitsVsY[year][xMass])->SetNameTitle(("1SigmaLimit_" + year + "_" + option + "_massX_" + std::to_string(int(xMass))).c_str(),  ("1 Sigma Limit - " + year + " " + option + " m_{Y} = " + std::to_string(int(xMass))).c_str());
                    std::get<0>(mapOfLimitsVsY[year][xMass])->SetMaximum(10000);
                    std::get<0>(mapOfLimitsVsY[year][xMass])->SetMinimum(1);
                    std::get<1>(mapOfLimitsVsY[year][xMass])->GetXaxis()->SetTitle("m_{Y} [GeV]");
                    std::get<1>(mapOfLimitsVsY[year][xMass])->GetYaxis()->SetTitle("#sigma(pp #rightarrow X) #times BR(Y(b#bar{b}) H(b#bar{b})) [fb]");
                    std::get<2>(mapOfLimitsVsY[year][xMass])->SetNameTitle(("2SigmaLimit_" + year + "_" + option + "_massX_" + std::to_string(int(xMass))).c_str(),  ("2 Sigma Limit - " + year + " " + option + " m_{Y} = " + std::to_string(int(xMass))).c_str());
                    std::get<0>(mapOfLimitsVsY[year][xMass])->SetMaximum(10000);
                    std::get<0>(mapOfLimitsVsY[year][xMass])->SetMinimum(1);
                    std::get<2>(mapOfLimitsVsY[year][xMass])->GetXaxis()->SetTitle("m_{Y} [GeV]");
                    std::get<2>(mapOfLimitsVsY[year][xMass])->GetYaxis()->SetTitle("#sigma(pp #rightarrow X) #times BR(Y(b#bar{b}) H(b#bar{b})) [fb]");
                    std::get<1>(mapOfLimitsVsY[year][xMass])->SetFillColor(kGreen);
                    std::get<2>(mapOfLimitsVsY[year][xMass])->SetFillColor(kYellow);
                    std::get<3>(mapOfLimitsVsY[year][xMass])->SetNameTitle(("ObservedLimit_" + year + "_" + option + "_massX_" + std::to_string(int(xMass))).c_str(), ("Observed Limit - " + year + " " + option + " m_{Y} = " + std::to_string(int(xMass))).c_str());
                    std::get<3>(mapOfLimitsVsY[year][xMass])->GetXaxis()->SetTitle("m_{Y} [GeV]");
                    std::get<3>(mapOfLimitsVsY[year][xMass])->SetMaximum(10000);
                    std::get<3>(mapOfLimitsVsY[year][xMass])->SetMinimum(1);
                    std::get<3>(mapOfLimitsVsY[year][xMass])->GetYaxis()->SetTitle("#sigma(pp #rightarrow X) #times BR(Y(b#bar{b}) H(b#bar{b})) [fb]");
    ...
    for(const auto& signal : signalList)
    {
        ...
        for(const auto & year : yearList)
        {
            if((year == "2017" || year == "RunII") && xMass == 1000 && yMass == 150) continue; // TMP !!!!!!!!!!!!!!!!!!!!!
            std::vector<std::string> limitOptions;
            if(year == "RunII") limitOptions = minLimitOptions;
            else limitOptions = allLimitOptions;
            for(const auto & option : limitOptions)
            {
                ...
                float limitObserved = -1.;
                // std::cout<<__LINE__<<std::endl;
                if(theTree->GetEntries() == 6) theTree->GetEntry(5); limitObserved  = limit*multiplier;
                ...
                theSignalMap2Dmap["LimitMapObserved"  ]->Fill(xMass,yMass,limitObserved  );
                // std::cout<<__LINE__<<std::endl;
                ...
                {
                // std::cout<<__LINE__<<std::endl;
                    std::get<0>(mapOfLimitsVsY[year][xMass])->SetPoint     (std::get<0>(mapOfLimitsVsY[year][xMass])->GetN()  , yMass, limitCentral);
                // std::cout<<__LINE__<<std::endl;
                    std::get<1>(mapOfLimitsVsY[year][xMass])->SetPoint     (std::get<1>(mapOfLimitsVsY[year][xMass])->GetN()  , yMass, limitCentral);
                // std::cout<<__LINE__<<std::endl;
                    std::get<2>(mapOfLimitsVsY[year][xMass])->SetPoint     (std::get<2>(mapOfLimitsVsY[year][xMass])->GetN()  , yMass, limitCentral);
                // std::cout<<__LINE__<<std::endl;
                    std::get<1>(mapOfLimitsVsY[year][xMass])->SetPointError(std::get<1>(mapOfLimitsVsY[year][xMass])->GetN()-1, 0., 0., limitCentral - limit1SigmaDown, limit1SigmaUp - limitCentral);
                // std::cout<<__LINE__<<std::endl;
                    std::get<2>(mapOfLimitsVsY[year][xMass])->SetPointError(std::get<2>(mapOfLimitsVsY[year][xMass])->GetN()-1, 0., 0., limitCentral - limit2SigmaDown, limit2SigmaUp - limitCentral);             
                // std::cout<<__LINE__<<std::endl;
                    std::get<3>(mapOfLimitsVsY[year][xMass])->SetPoint     (std::get<3>(mapOfLimitsVsY[year][xMass])->GetN()  , yMass, limitObserved);
                // std::cout<<__LINE__<<std::endl;
                }
                // std::cout<<__LINE__<<std::endl;
            }
    ...
    for(const auto& year : yearList)
    {
        ...
        for(const auto& option : limitOptions)
        {
            ...
            std::get<0>(theFullMapOfPlots[year][option])["LimitMapObserved"  ]->Write();
            for(const auto yMass : yMassList)
            {
                // std::cout<<__LINE__<<std::endl;
                ...
                std::get<3>(std::get<1>(theFullMapOfPlots[year][option])[yMass])->Write();
            }
            if(option == "syst" || option=="freezeBKGnorm")
            {
                for(const auto xMass : xMassList)
                {
                // std::cout<<__LINE__<<std::endl;
                    std::get<0>(mapOfLimitsVsY[year][xMass])->Write();
                    std::get<1>(mapOfLimitsVsY[year][xMass])->Write();
                    std::get<2>(mapOfLimitsVsY[year][xMass])->Write();
                    std::get<3>(mapOfLimitsVsY[year][xMass])->Write();
                }
            }
                // std::cout<<__LINE__<<std::endl;
            outputFile.cd();
