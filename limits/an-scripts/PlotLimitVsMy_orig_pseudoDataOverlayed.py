...
parser.add_argument('--pseudo'    , dest = 'pseudo'    , help = 'pseudo tag' , required = True)
...
for theMass in massList:
    ...
    if theMass == 650:
        ...
        x.append(350)
        y.append(val)
        grPseudo = TGraph( 1, x, y )
        grPseudo.SetMarkerStyle(5)
        grPseudo.SetMarkerColor(ROOT.kRed+2)
        grPseudo.Draw("psame")
    ...
    theLegend.AddEntry(grPseudo, "Pseudo Data Set"                             , "p" )
    ...
    odir = "results/Limits_{0}/".format(args.pseudo)
