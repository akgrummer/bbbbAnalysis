...
void Plot2DLimitMap(TString tag, std::string year, std::string option = "syst")
{
    ...
    TString inputFileName = "hists/Signficance/Limits_"+tag+".root";
    ...
    TCanvas* theCanvas = new TCanvas(canvasName.c_str(), canvasName.c_str(), 800, 600);
    gPad->SetTickx(1);
    gPad->SetTicky(1);
    gPad->SetMargin(0.12,0.16,0.12,0.09);
    limitMap->Draw("colz");

    std::string plotTitle = "Signficance " + year;
    ...
    limitMap->GetZaxis()->SetTitle("#sigma");
    limitMap->GetZaxis()->SetTitleFont(62);
    limitMap->GetZaxis()->SetTitleSize(0.045);
    // limitMap->SetMinimum(-0.01);
    // limitMap->SetMaximum(2000.);
    // limitMap->SetMaximum(10.);
    limitMap->GetZaxis()->SetRangeUser(0.,5.1);
