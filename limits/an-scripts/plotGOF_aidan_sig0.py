...
def doPlots(config):
    ...
    saveName = "GoF_{0}_{1}_{2}_sig0".format(algorithm, masspoint, config["year"])
...
if __name__ == "__main__":
    ...
    odir = "GOFfiles/GOFfiles_sig0/GOFPlots_{0}/"
    with open("{0}PVALS_{1}_{2}_sig0.csv".format(odir.format(args.tag),args.year,args.algo), "w") as pvalsFile:
        ...
        for signalRaw in open(limitWorkDir+"/"+args.samplelist, 'rb').readlines():
            ...
            config = {
                ...
                "fData": "GOF_{0}_{1}_{2}_DATA_sig0.root".format(args.year, sig, args.algo),
                "fToy": "GOF_{0}_{1}_{2}_TOYS_sig0.root".format(args.year, sig, args.algo),
                "PlotLabel": "{0}".format(masspoint),
