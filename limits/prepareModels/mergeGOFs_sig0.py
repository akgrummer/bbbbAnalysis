...
def mergeFiles(sig):
    folderName = "GOFfiles/GOFfiles_sig0/GOFPlots_" + tag
    cmd='mkdir -p ' + folderName
    runCMD(cmd)
    outputFileName = folderName + "/GOF_{0}_{1}_{2}_TOYS_sig0.root".format(year,sig,algo)
    # USING SIG0 tag for frozen signal strength runs - need to exclude the sig0 files if going back to regular sig GOFs
    haddCmd = "hadd -f0 " + outputFileName + " `xrdfs root://cmseos.fnal.gov ls -u /store/user/" + username + "/bbbb_limits/" + tag + "/" +"gofFiles_{}".format(year) + " | egrep 'GOF_{0}_{1}_{2}_NTOYS.*sig0\.root'`".format(year,sig,algo)
    runCMD(haddCmd)
    # USING SIG0 tag for frozen signal strength runs - need to exclude the sig0 files if going back to regular sig GOFs
    runCMD('xrdcp -f -s {0}/{1} {2}/{1}'.format(outputDir, 'GOF_{0}_{1}_{2}_DATA_sig0.root'.format(year,sig,algo), folderName))
